...
/**
 * Splits raw markdown into alternating segments of plain text and fenced
 * code blocks in one of DIAGRAM_LANGUAGES, so a document viewer can render
 * those blocks as diagrams while leaving everything else as plain text.
 *
...
export function splitMarkdownFences(markdown: string): MarkdownSegment[] {
  ...
  while ((match = fenceRe.exec(markdown)) !== null) {
    ...
    segments.push({ type: lang.toLowerCase() as DiagramLanguage, content: body.trim() });
