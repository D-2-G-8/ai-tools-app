 ...
 * references must have a matching `.<name>` top-level selector in the
 * generated stylesheet. Catches the exact failure mode two independently
 * generated files are prone to (a class name that doesn't match) before
 * anything is committed -- see this module's top comment.
 */
export function checkClassNamesMatch(tsxContent: string, cssContent: string): ClassNameCheckResult {
  ...
  for (const m of cssContent.matchAll(/^\s*\.([A-Za-z_-][A-Za-z0-9_-]*)\s*[,{:]/gm)) defined.add(m[1]);
